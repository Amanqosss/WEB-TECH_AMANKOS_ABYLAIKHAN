...
-- We use WHERE NOT EXISTS because the film table has no UNIQUE
-- constraint on title, so ON CONFLICT is not available.
-- language_id is resolved dynamically via subquery to avoid
-- hardcoding IDs that may differ across database instances.
-- ============================================================
...
-- We use WHERE NOT EXISTS for actor inserts (no UNIQUE constraint
-- on first_name + last_name combination).
-- For film_actor we use ON CONFLICT DO NOTHING because the table
-- has a composite primary key (actor_id, film_id), making it
-- the correct and efficient idempotency strategy.
-- All FK IDs are resolved via subqueries — no hardcoded values.
-- ============================================================
...
-- store_id and film_id are both resolved dynamically.
-- We pick store_id = MIN(store_id) to avoid hardcoding.
...
-- We update an existing customer who has >= 43 rentals and
-- >= 43 payments. Using a subquery to identify the customer_id
-- with the highest rental count ensures we target the right row
-- without hardcoding an ID.
-- address_id is also resolved dynamically (MIN existing address).
-- ============================================================
...
-- We first SELECT to preview affected rows, then DELETE.
-- Only payment and rental are cleaned — customer and inventory
-- are intentionally preserved as required by the assignment.
-- customer_id is resolved dynamically by name to avoid hardcoding.
-- ============================================================
...
-- inventory_id, customer_id, and staff_id are all resolved via
-- subqueries — no hardcoded IDs anywhere.
-- return_date is calculated as rental_date + rental_duration days.
-- payment_date is set to 2017 H1 to satisfy partition constraints.
-- RETURNING is used on the first rental INSERT to demonstrate
-- the feature and confirm the inserted row.
-- WHERE NOT EXISTS guards against duplicate rows on re-run.
-- ============================================================
